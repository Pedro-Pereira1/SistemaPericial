...
const Metrics: React.FC = () => {
    ...
    const explanationImages: { [key: string]: { src: string; name: string; description: string }[] } = {
        ...
        'lightgbm': [
            { src: '/images/lgbm_1.png', name: 'Interaction Value Plot for Network Traffic Features', description: 'This SHAP interaction summary plot for the LightGBM (LGBM) model highlights the importance and behavior of features such as Total Backward Packets, Forward Packet Length Std, Flow Duration, and Forward Packet Length Mean, which exhibit wide distributions of SHAP values, indicating significant and varying contributions to the model\'s predictions. Features like Forward Packet Length Max and Backward Packets Length Total show distinct red and blue clusters, suggesting strong interactions with other features that impact predictions differently across classes (e.g., attack vs. normal). Features with tightly clustered SHAP values near zero, such as Total Forward Packets, indicate limited or context-specific contributions. These insights underscore the critical role of certain features in distinguishing between network traffic patterns while also highlighting potential redundancies in others, offering valuable guidance for refining the model and understanding its decision-making process.' },
            { src: '/images/lgbm_2.png', name: 'SHAP Summary Plot', description: 'This SHAP summary plot shows the importance of features in a multi-class classification model for identifying network activities. The features are sorted by importance, with Init Fwd Win Bytes and Fwd Seg Size Min being the most impactful. The horizontal axis represents the average SHAP values (impact on predictions), while the stacked bar colors show how each feature contributes to different classes. Top features like Init Fwd Win Bytes significantly influence specific classes (e.g., DDoS), while others contribute across multiple classes. This plot helps interpret the model by highlighting which features are most relevant for detecting network anomalies or benign traffic.' },
        ],
        'cnn_rnn': [
            { src: '/images/not_available.jpg', name: 'Not Available', description: 'Image Not Available' },
            { src: '/images/not_available.jpg', name: 'Not Available', description: 'Image Not Available' },
                ...
                <div className="explicable-image-container">
                    ...
                    {isFullscreen && (
                        <div className="fullscreen-overlay" onClick={toggleFullscreen}>
                            ...
                            <img
                                className={`explicable ${
                                    explanationImages[selectedExplanationModel][currentImageIndex].src.includes("_2")
                                        ? "small-image"
                                        : "large-image"
                                }`}
                                src={explanationImages[selectedExplanationModel][currentImageIndex].src}
