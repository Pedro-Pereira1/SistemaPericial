...
interface Evidences {
    alertId: string;
    trafficSpike: string | null;
    legitimateSource: string | null;
    abnormalPattern: string | null;
    mitigationEnabled: string | null;
    contactISP: string | null;
  }
...
const base:Evidences = {
    alertId:"DDoS",
    trafficSpike: "null",
    legitimateSource: "null",
    abnormalPattern: "null",
    mitigationEnabled: "null",
    contactISP: "null"
...
const startProcess = () => {
  ...
  const convertToJsonFormat = (data: Evidences): { fact_name: string; variables: (number | string)[] } => {
    return {
        ...
        variables: [
            "DDoS",
            data.trafficSpike ?? "null",
            data.legitimateSource ?? "null",
            data.abnormalPattern ?? "null",
            data.mitigationEnabled ?? "null",
            data.contactISP ?? "null"
        ]
    };
  }
...
const fetchWhyExplanation = async () => {
  if ((expertSystem === 'Drools' || expertSystem === 'Prolog') && currentQuestion) {
    try {
      const explanationList = await AlertService.getWhyExplanation(alertResponse,evidences, expertSystem,"CMF");
...
const fetchWhyNotExplanation = async () => {
  if ((expertSystem === 'Drools' || expertSystem === 'Prolog') && currentQuestion) {
    try {
      const conclusions = await AlertService.getPossibleConclusions(expertSystem,"CMF");
